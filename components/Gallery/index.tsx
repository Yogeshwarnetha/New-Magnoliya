...
import { getGalleryImages } from '../../apirequests/gallery';

// Define the interface based on your backend model
...
const Gallery = () => {
  ...
  // Gallery categories - matching backend categories
  const categories = [
  ...
  // Fetch gallery data from API (using shared API helper)
  const fetchGalleryData = async (category?: string) => {
    try {
      setIsLoading(true);

      // use the shared apirequests helper; pass undefined when category is 'all'
      const cat = category && category !== 'all' ? category : undefined;
      const res = await getGalleryImages(cat);

      if (res.ok && res.data) {
        // Map API response to local GalleryItem shape. The API helper's GalleryImage
        // type doesn't include our `type` field, so default to 'image' when missing.
        setGalleryItems(res.data.map((d: any) => ({
          ...d,
          type: d.type ?? 'image',
        })));
      } else {
        console.error('Failed to fetch gallery images', res.error);
        setGalleryItems([]);
      }
    } catch (error) {
  ...
  const nextImage = () => {
    ...
    setSelectedImage(filteredImages[nextIndex].image);
    setZoomLevel(1);
    setPosition({ x: 0, y: 0 });
  };
  ...
  const prevImage = () => {
    ...
    setSelectedImage(filteredImages[prevIndex].image);
    setZoomLevel(1);
    setPosition({ x: 0, y: 0 });
  };
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                  {filteredImages.map((item, index) => (
                    ...
                    >
                      <img
                        src={item.image}
                        alt={item.caption || item.image_title || `Gallery image ${item.id}`}
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-end justify-center pb-6">
                        <div className="text-center text-white transform translate-y-8 group-hover:translate-y-0 transition-transform duration-500">
                          <h3 className="font-semibold text-lg mb-2">
                            {item.caption || item.image_title || `Image ${item.id}`}
                          </h3>
